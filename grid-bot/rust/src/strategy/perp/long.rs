...
pub struct PerpLongStrategy;

impl GridStrategy for PerpLongStrategy {
    ...
    fn validate_config(&self, config: &GridConfig) -> Result<()> {
        debug_assert_eq!(config.perp_mode, PerpMode::Long);
        config.validate()
    }
...
pub static STRATEGY: PerpLongStrategy = PerpLongStrategy;
